...
OUTPUT_SLIDES_DIR = f"../data/pdf"

FRAME_RATE = 3  # no.of frames per second that needs to be processed, fewer the count faster the speed
TEXT_COMPARE_RATE = 10  # every TEXT_COMPARE_RATE-th frame will be ocr recognized and compared with previously captured
WARMUP = FRAME_RATE  # initial number of frames to be skipped
FGBG_HISTORY = FRAME_RATE * 5  # no.of frames in background object
VAR_THRESHOLD = 10  # Threshold on the squared Mahalanobis distance between the pixel and the model to decide whether
...
MIN_PERCENT = 1  # min % of diff between foreground and background to detect if motion has stopped
...
def detect_unique_screenshots(video_path, output_folder_screenshot_path):
    ...
    output_dir_path = Path(output_folder_screenshot_path)
    logger.info(f"output_dir_path = {output_dir_path.absolute()}")
    output_dir_path.mkdir(parents=True, exist_ok=True)
    if not output_dir_path.is_dir():
        logger.warning(f"Directory not exist {output_dir_path}")
    ...
    logger.info(f'{screenshoots_count} screenshots Captured!')
    logger.info(f'Time taken {time.time() - start_time}s')

    return frames_filenames


def initialize_output_folder(video_path, output_folder_screenshot_path="../static/images"):
